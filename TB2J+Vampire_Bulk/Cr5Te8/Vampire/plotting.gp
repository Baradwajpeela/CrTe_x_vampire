...
plot "output_MAE" using 1:5 with points pt 5 title "MAE = -1.72 meV", \
     m1(x) with lines lw 2 lc rgb "blue" title sprintf("Fit MAE: Tc=%.2f, beta=%.2f", Tc1, beta1), \
     "output_no_MAE" using 1:5 with points pt 5 title "MAE = 0 meV", \
     m2(x) with lines lw 2 lc rgb "red" title sprintf("Fit No MAE: Tc=%.2f, beta=%.2f", Tc2, beta2)
